...
class DiskUsageCollector(diamond.collector.Collector):
    ...
    def get_disk_part_name(self, device):
        """
        Return the partition partition label if exists
        """
        context = pyudev.Context()

        try:
            part_name = pyudev.Device.from_name(context, 'block', device).get('ID_PART_ENTRY_NAME').decode('string-escape')

        except:
            return

        if part_name:
            return re.sub(r'[\s+\.]+', '-', part_name)
            ...
            try:
                for line in fp:
                    try:
                        ...
                        if self.config['attach_device_label']:
                           partlabel = self.get_disk_part_name(device)

                           if partlabel:
                             result[(major, minor)].update({'partlabel': partlabel})
    ...
    def collect(self):
        ...
        for key, info in results.iteritems():
            ...
            for key, value in info.iteritems():
                if key == 'device' or key == 'partlabel':
                    continue
                oldkey = key
            ...
            if (metrics['io'] > 0 or self.config['send_zero']):
                ...
                if self.config['attach_device_label'] and 'partlabel' in info:
                    device = '%s-%s' % (info['device'], info['partlabel'])
                else:
                    device = info['device']

                for key in metrics:
                    metric_name = ".%s.%s" % (device, key)
